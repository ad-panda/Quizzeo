...
$nom_entreprise = $_SESSION['nom_compte'];
$quiz_list = [];

// Récupérer les quiz créés par cette entreprise et le nombre de participations
$sql = "
    SELECT 
...
<head>
    <meta charset="UTF-8">
    <title>Dashboard Entreprise - Quizeo</title>
    ...
    <style>
        ...
        .btn-creer { padding: 10px 20px; background-color: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer; }
    </style>
</head>
<body>
    <div class="dashboard-content">
        <h1>Tableau de Bord Entreprise 🏢</h1>
        <p>Bienvenue **<?php echo htmlspecialchars($nom_entreprise); ?>**. Créez vos questionnaires d'évaluation.</p>
        
        <button class="btn-creer" onclick="location.href='creation_quiz.php'">Créer un nouveau Quiz</button>
        
        <h2 style="margin-top: 30px;">Mes Questionnaires (<?php echo count($quiz_list); ?>)</h2>
            ...
            <?php foreach ($quiz_list as $quiz): ?>
                <div class="quiz-card">
                    ...
                    <?php elseif ($quiz['statut'] === 'termine'): ?>
                        <button onclick="location.href='voir_resultats_entreprise.php?id=<?php echo $quiz['id_quiz']; ?>'">Voir les Pourcentages de Réponses</button>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
